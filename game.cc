...
namespace main_savitch_14
{
	...
	game::who game::play( ) {
	   ...
	   // The commenting you see below sets this up for Phase One
		passes = 0;
		while (!is_game_over() && passes < 2) {
			display_status();
			// check if player has any legal moves: if not incremnet move_number and skip their turn
...
//					make_human_move();
				...
				passes++;
				move_number++;
				cout << endl << "No moves possible, passing turn." << endl;
			}
		}
		display_status();
		return HUMAN;
    ...
    void game::make_human_move( )
    {
		...
		move = get_user_move( );
		while (!is_legal(move)) {
			display_message("Illegal move, pleas enter a legal move in the from: F5\n");
			move = get_user_move( );
        }
		make_move(move);
    }
